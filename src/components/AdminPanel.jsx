  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in">
      ...
      <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-md border border-gray-200 dark:border-slate-700">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">➕ Add New Tracker</h2>
        <form onSubmit={handleAdd} className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <input 
            type="text" placeholder="Tracker Name (e.g., CoC War Log)" 
          ...
          />
          <input 
            type="text" placeholder="Google Sheet ID (from URL)" 
      ...
      <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-md border border-gray-200 dark:border-slate-700">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">⚙️ Manage Trackers</h2>
        {registry.length === 0 ? (
          <p className="text-gray-500 dark:text-slate-400">No trackers found.</p>
        ) : (
          <div className="space-y-3">
            {registry.map((tracker, index) => (
              <div key={index} className="flex justify-between items-center p-4 bg-gray-50 dark:bg-slate-900/50 rounded-lg border border-gray-100 dark:border-slate-700">
                <div>
                  <h3 className="font-bold text-gray-900 dark:text-white">{tracker['Tracker Name']}</h3>
                  <p className="text-xs text-gray-500 dark:text-slate-400 font-mono mt-1">ID: {tracker['Sheet ID']}</p>
                </div>
                <button 
