...
const TrackerCard = ({ tracker, onOpen }) => {
  ...
  useEffect(() => {
    const fetchCardData = async () => {
      try {
        ...
        // NEW: Parse the updated API response structure
  ...
  // NEW: Helper function to format the ISO date into a clean, readable string
  const formatTime = (isoString) => {
    if (!isoString) return '';
    const date = new Date(isoString);
    // Formats to something like: "Oct 24, 2:30 PM"
  ...
  return (
    <div className="flex flex-col justify-between p-6 bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-md hover:shadow-xl transition-all duration-300 text-gray-800 dark:text-white">
      ...
      <div className="flex-grow flex flex-col justify-center py-4 border-y border-gray-100 dark:border-slate-700/50 my-4 min-h-[120px]">
        ...
        {error && (
          <p className="text-xs text-red-500 dark:text-red-400 text-center bg-red-50 dark:bg-red-500/10 p-2 rounded break-words">
            Error: {error}
          </p>
        )}
      ...
      {/* NEW: Updated Footer to show both Record Count and Last Updated Time */}
      <div className="flex justify-between items-end mt-2">
        <div className="flex flex-col">
          <span className="text-xs font-medium text-gray-500 dark:text-slate-400">
            {sheetData.length > 1 ? `${sheetData.length - 1} Records` : 'No Records'}
          </span>
          {lastUpdated && (
            <span className="text-[10px] text-green-500 dark:text-green-500 font-bold mt-1">
              Modified: {formatTime(lastUpdated)}
            </span>
          )}
        </div>
        <a 
          ...
          rel="noreferrer"
          onClick={() => onOpen(id)} 
          className="text-sm text-blue-500 dark:text-blue-400 hover:text-blue-600 dark:hover:text-blue-300 flex items-center gap-1 transition-colors font-medium mb-0.5"
        >
          Open Sheet <span className="text-lg">↗</span>
        </a>
      </div>
